...
function applyUILang() {
    ...
    const editHistTitle = document.querySelector('.edit-history-title');
    if (editHistTitle) editHistTitle.textContent = t('editHistory');
    const submitBtn = document.getElementById('btnSubmitChanges');
    if (submitBtn && getPendingCorrections().length === 0) submitBtn.textContent = t('noPendingChanges');
...
function renderEditHistory() {
    ...
    if (submitted.length > 0) {
        const header = document.createElement('div');
        header.className = 'edit-section-header';
        header.textContent = `Submitted (${submitted.length})`;
        body.appendChild(header);
    }
