...
function renderEditHistory() {
    ...
    const submitted = all.filter(c => c.submitted);

    if (pending.length > 0) {
        const header = document.createElement('div');
        header.className = 'edit-section-header';
        header.textContent = `${t('pending')} (${pending.length})`;
        body.appendChild(header);
    }
    pending.forEach((item, idx) => {
        const realIdx = all.indexOf(item);
        body.appendChild(createEditCard(item, realIdx, false));
    });

    if (submitted.length > 0) {
        const header = document.createElement('div');
        header.className = 'edit-section-header';
        header.textContent = `${t('submitted')} (${submitted.length})`;
        body.appendChild(header);
    }
    submitted.forEach((item) => {
        const realIdx = all.indexOf(item);
        body.appendChild(createEditCard(item, realIdx, true));
    });
