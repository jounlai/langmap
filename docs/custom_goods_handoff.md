...
> **ロケールと表示言語（重要・Makoto 側の要件）**
> - Makoto の店舗ロケールは **`ja` / `en` のみ**（`localePrefix: "always"`、既定 `ja`）。送り出し側の UI 言語が `ja` 系なら `/ja/`、それ以外は `/en/` に振る（フランス語・韓国語 UI も店舗 UI は `/en/`）。**`/ja/` は必ず日本語で表示されること**（日本語 UI から来て英語ページになる不具合は、この特設ページが未ローカライズ／未実装なのが原因）。
> - **言語名は必ずクエリの `name` / `names`（送り出し側 UI 言語にローカライズ済み）を表示に使う**こと。店舗ロケール（en）で自前に引き直して英語名にしないこと。例：フランス語 UI からは `name=Japonais`、日本語 UI からは `name=日本語(標準)` が届く（LangMap 側で UI 言語の言語名テーブルを読み込んでから送る）。
> - **単語（概念）ラベルの言語**：ユーザーの UI 言語（`ui`）に合わせるのが望ましい。`ui` が `ja`/`en` 以外（例 `fr`）のとき、概念ラベル（water→eau など）を店舗ロケールの英語のままにしない。Makoto 側で概念ラベルの多言語表を持つか、LangMap 側から UI 言語の概念ラベルを渡す拡張が必要（未実装。必要なら別途）。単語そのもの（対象言語の綴り＋IPA）は翻訳しない。

| 位置 | 名前 | 説明 | 例 |
|---|---|---|---|
| path | `{locale}` | Makoto のロケール（`ja` / `en` のみ）。送り出し側 UI 言語が `ja` 系→`ja`、他→`en`。 | `ja` |
