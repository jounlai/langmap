...
12. **RESOLVED 2026-09-04 — /sitemap-seo.xml is a sitemap index.** It was one file at 49,761,107 of 52,428,800 bytes (94.9%) and only 23,161 of the 50,000 URLs, so the BYTE limit bound first: each `<url>` carries 20 xhtml:link alternates (19 UIs + x-default) at ~2,149 bytes against a ~60-byte `<loc>`. A sitemap over the limit is not truncated — the whole file is rejected — so one ordinary batch of new languages would have silently dropped all 23,161 SSR pages out of the declared index at once.

   `/sitemap-seo.xml` now emits a `<sitemapindex>`; the URLs live in `/sitemap-seo-1.xml … -5.xml`, `SEO_SITEMAP_CHUNK = 250` source pages each (× 19 UIs = 4,750 URLs, ~10.1 MB, 19.3% of the limit). `index.php` routes `/sitemap-seo-<n>.xml` and passes `$seo_sitemap_part`. Adding languages now adds PARTS rather than bytes, so the cliff is gone unless the per-URL cost itself grows several-fold — a completely full part is 5.1x under the limit. Verified byte-for-byte against the old single file: the same 23,161 `<loc>`s in the same order and the same 463,220 alternates.

   `sitemap_size_check.js` now measures the index and every part, reports the WORST part (the one that would be rejected first), and fails if a part exists that the index does not list, or the index lists a part that produces nothing.

13. **(b) and (c) RESOLVED 2026-09-04; (a) still open.** (b) `tree.html` and `poster.html` had NO heading at all — not one h1–h6 — so a screen-reader user had nothing to navigate by. Both now carry the visually-hidden `<h1>` the other four pages already use (same inline style, so zero layout risk; measured at 1x1 px in a browser, no horizontal overflow, no page errors). (c) `wordmap.html` and `hanmap.html` jumped h1 to h3: the `<h3 class="trivia-compare-title">` in the comparison dialog, which is that dialog's own top-level heading and is now `<h2>`. Visually identical — the class sets font-size, font-weight and margin explicitly, which are exactly the three properties where the h2 and h3 UA defaults differ; confirmed by rendering both tags with that class in a browser and diffing the computed styles.

   New guard `tools/heading_order_check.js` ("page headings" in check_all): every page needs exactly one `<h1>` and headings may not skip a level. Verified it fails on both shapes — a page whose h1 was demoted, and an h1 followed by an h4.

   Also fixed while in tree.html: it loaded `styles.css?v=130` **twice**, two identical `<link>` tags on consecutive lines.

   **(a) `dir="rtl"` is still not set** by index.html, wordmap.html, hanmap.html or namemap.html for ar/he, though `seo/lib.php` and `tree.html` do set it. This one really does need a judgement: it mirrors the entire map UI, and index.html carries a deliberate note that Arabic and Hebrew SENTENCE data is displayed left-to-right for comparison, which the document direction would fight.
...
30. **Findings from the 2026-09-06 historical-coverage sweep — four of them are data defects, not gaps.**

   The sweep itself filled 42 cells across black, wine, wheel, milk, bear, honey, iron, hundred, rain and wind (three commits). The tooling is in `~/langmap-work/histfill/`: `harvest.py` runs `incategory:"<Language> lemmas" <gloss>` per gap, `etym.py` reads a proto-form off a modern descendant's `{{inh}}` template (reconstruction page titles are unguessable; descendants name their ancestor), `direct.py` verifies a named lemma, `apply.py` inserts cells next to a named anchor. `rows.json` holds every historical row's current cells, which is what makes the IPA derivable.

   **(a) `txb`, `xto` and `kho` write Brahmi surfaces that are inconsistent back-transliterations of Latin scholarly forms, not manuscript spellings.** txb blood is 𑀬𑀲𑀭 = ya-sa-ra for *yasar* (the manuscripts write ysāre); xto earth 𑀢𑁆𑀓𑀁 uses anusvara for final -m while txb bone 𑀆𑀬𑁆 uses virama for final -y; kho eye 𑀘𑁂𑀰𑁆𑀫𑀦𑁆 uses virama for final -n. Three conventions in rows that share a script. This is why `txb`/`xto` were left empty for black although the words are known (erkent, arkant) and for iron although xto añcu is attested: any spelling added would be a fourth convention. Settling it needs a decision about what these surfaces are FOR before any more cells go in.

   **(b) The `zh_han` Han Map row contains Baxter–Sagart Old Chinese forms in what is a Later Han row.** `night [ɢ]Ak-s`, `three s.rum`, `tongue mə.lat`, `tooth m-ɢˤra` are `och` values sitting in the row that otherwise transcribes Schuessler's Later Han (`bird teuʔ`, `eat ʑiək`). Four cells, and they are not a transcription difference — they are the wrong stage of the language.

   **(c) The Chinese historical rows cannot be filled from Wiktionary.** `och` uses Baxter–Sagart, `zh_han` Schuessler's Later Han, `zh_tang` Baxter's Middle Chinese, `zh_song` an Early-Mandarin stage with Chao tones. Wiktionary's Chinese entries render Zhengzhang for Old Chinese and often no Baxter–Sagart at all (酒 has only `*ʔsluʔ`), so pulling from there would mix reconstruction systems inside one row — the same defect as (b). `zh_han` is missing twelve of the fifteen sweep concepts; all of them need the published Baxter–Sagart 2014 list and Schuessler's *Minimal Old Chinese and Later Han Chinese*, not a web lookup.

   **(d) The Word Map `th` row's IPA is internally inconsistent.** Three notations coexist: Chao letters (`saːm˩˩˦`), acute/grave diacritics (`nók`, `tɕamùːk`), and no tone at all (`din`). The rising tone is written `˩˩˦` in ขาว and `˩˧` in หมา and ฝน; the high tone `˧˥` in น้ำ and `˦˥` in ร้อย. Roughly 40 cells. The Han Map's new `th` row uses one set throughout (mid ˧, low ˨˩, falling ˥˩, high ˦˥, rising ˩˩˦) and took 三/四/五 from the Word Map cells that already agree with it.

   **Still open in the sweep, with the reason:** `paus` milk (PAn *susu is glossed 'breast'; asserting it as 'milk' is a step the source does not take), `p_ine` milk (*glakt has no reconstruction page and the etymon is disputed), `p_ine` iron and cat (post-PIE — a genuine absence worth stating rather than filling), `uga` salt (𐎎𐎍𐎈𐎚 found, no vocalisation derivable from the row), `hit` daughter (𒌉𒊩 is a Sumerogram), `hbo` cat (Biblical Hebrew has no word for it), `osp` and `kaw` wine (no Wiktionary section for the language).

31. **Han Map `th` (Thai) is deliberately sparse and must stay that way unless the criterion changes** — added 2026-09-06 on a reader request. Ten characters (二三四五六七八九十馬), each one whose Thai word Wiktionary attributes to an Old Chinese or Late Middle Chinese source. It is a LOAN LAYER, not a reading system: Thai never used the characters, and the row's description says so in all 19 UI languages. Do not fill it out to 61 by adding Thai's native word for each character's meaning — that is what `za` and `mnc` do, and it would make the row claim something different from what its own description claims. 一 (native หนึ่ง) and 龍 (มังกร, from Sanskrit makara) are excluded on purpose. The later Teochew stratum belongs to `nan_th`, which already exists; none of its characters (桌, 粿條, 交椅, 叔, 肉麵) is in the 61 anyway. `native_script_check.js` now knows the row must be Thai script.

20. **Two work queues opened by the 齒 reader report (2026-09-05).** 酒井亨 reported that Taiwanese says 齒 khí, not 牙 gê. It was one cell; fixing it exposed ten more in `nan` alone and two error classes worth checking mechanically.

   **`tools/sinitic_tone_outlier_check.js` — 126 left.** A Chao contour a row uses EXACTLY ONCE, in a row of 30+ tone tokens, is almost always a neighbour's value. `nan` carried Mandarin's ˧˥ and ˥˩ (hello, i, moon, sun, we, you) and Xiamen's ˨˨ and ˦˦ (thanks, milk) while `nan_xm` had the right value throughout. All ten fixed; the report names which row uses the contour freely, so each finding is actionable. Leaders: zh_db 13, wuu_jx 12, wuu_hz 11, wuu_sz 9, wuu 9, nan_qz 9, nan_pn 9, czh 9. Tuned to "used exactly once" on purpose — at "twice or fewer" it returns 244 and starts catching real notational variation.

   **`tools/sinitic_lexical_import_check.js` — 29 to review.** A row whose surface equals Mandarin's where two or more of its own siblings differ. Found 爸爸/媽媽 in `nan` against nan_xm's 老爸/老母 (the MOE Taiwanese dictionary has no 爸爸 entry at all). Traditional/simplified is normalised away first, or 41 findings become 20 and half of the rest are `hak_cn` 铁 against `hak_tw` 鐵, which is the documented per-code script convention working correctly.

   **The direction is not always the same, and that is the trap.** The biggest cluster is `sun`: yue, wuu, wuu_sz and cjy_xz all say 太陽 where every sibling says 日頭 — which looks like four flagship rows filled from Mandarin, until you check and find Wiktionary glosses Cantonese 日頭 as DAYTIME. In the Yue group the flagship row is right and the five dialect rows are the suspects. Nothing on this list should be changed without a dictionary for that specific lect. `nan` you 你 against six siblings' 汝 is on the list and is CORRECT (你 is the MOE's recommended character for lí) and is exempted by name.
