...
16. **Four `meta_i18n/` slices are for UI languages no page can select (~998 KB)** — found 2026-08-27 (review 464). The directory holds 22 files where 18 would do: `es_eu.js`, `es_mx.js`, `pt_eu.js` and `pt_br.js` have no route to them. `resolveUiLang()` gates every source on `UI_I18N[x]` existing, and the cookie branch explicitly strips the suffix (`tree.html:413`: `const base = cookie.split('_')[0]`), so `es_mx` resolves to `es`; `?ui=` passes through the same gate. They exist because the description data carries 23 locales while the picker offers 19 — the same root as the `meta_desc/` item below. Two ways out: add the locales to the picker, or make `build_meta_split.js` emit only the UI set. Left alone because which one is right is a product call. `build_meta_split.js` currently spends time on them and `slice_version_check.js` hashes them.
