...
リンクには言語の指定が2か所あります。取り違えると、日本語の利用者に英語の
画面が出たりします。

| 指定 | 何を決めるか |
|---|---|
| URLのロケール（`/ja/` か `/en/`） | Makoto Gadgets の画面表示（ボタンや説明文） |
| `ui=` | 単語の意味と言語名をどの言語で出すか |

LangMap の画面が日本語なら `/ja/…&ui=ja`、英語なら `/en/…&ui=en` のように
両方そろえてリンクしてください。

そろえた上で、Makoto 側のヘッダーにある JP / EN は利用者が自由に切り替え
られます。切り替えても `ui=` はそのままなので、「単語の意味は英語のまま、
サイトの表示だけ日本語」という見方ができます（こちらでそう作ってあります）。
