...
### 保留・未適用
- `zh_kf 目 surface "mu⁴²"` — 適用済確実件（次濁入→陰平 24、ipa ˨˦）と**矛盾**のため除外。
- `hak_hl 坐 chhô` / `nan_th 五 文讀 ngou6` — 表層ローマ字バケット（オーナー見送り）。
- `yue_gz 中:1/中:2 ipa` — char キー不整合＋tie-bar 除去のみで rationale 空のため除外。
