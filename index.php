...
if ($map === 'wordmap' || $map === 'hanmap') {
    ...
    // A language whose code we corrected keeps its old URL working, but the
    // 301 now happens inside seo/{map}.php, after that map's own lookup has
    // failed — the two maps do not share a code space, and pko/pst/paa/phm are
    // live Han Map codes while the Word Map has moved them to p_kor/p_sit/etc.
    header('Content-Type: text/html; charset=utf-8');
