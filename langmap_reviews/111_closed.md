...
100文 × 223言語 = 22,300 セルの語順データを、言語族で分割した **30レビュワー**（実働28）が監査し、**10開発者**が懐疑的検証で apply/reject を判定した大規模ラリー。110回の既存レビュー後の残存欠陥を対象とし、量より精度を重視。

- **総 findings: 142**（確実 85 / 蓋然 40 / 要検討 17）
- **開発者検証: 142 件**（apply 承認 103 / reject 39）
- **本レビューで適用: 確実 78 件**（data.js 反映済み）
- **owner確認待ち（未適用）: 25 件**（蓋然/要検討 — register・方言・owner判断の余地）
...
各言語族の sibling 多数派・en/ja 参照・確立ルール（feedback_cross_sibling_role_consistency / no_character_level_split / split_segments_like_reference / no_adjacent_same_segments / modal_verb_not_subject 等）に基づく確定修正。surface text は再分節時は不変。

### #88 `af` — Verified B+C fusion, pure re-segmentation, surface text unchanged ('KI is besig om die wêreld te verander')
...
### #21 `ar_iq` — Verified defect: current ar_iq fuses future particle راح + dinner-verb أتعشه into one E segment, dropping D
...
### #90 `as` — Confirmed transliteration error: as B প্ৰগ্ৰেমিং has medial ে (U+09C7, 'e'), wrong for English 'programming' which takes া (U+09BE, ā)
...
### #57 `bho` — Confirmed defect: current D fuses manner-adverb धड़ल्ले से (fluently) with verb बोलेली, leaving palette C empty
...
### #89 `ca` — Verified orthographic defect
...
### #1 `chr` — Verified defect: current chr A = ᎠᏋ (U+13A0 U+13CB, 'a'+'quv'); standard Cherokee 1sg 'I' is ᎠᏴ (U+13A0 U+13F4, 'a'+'yv')
...
### #9 `chr` — Same verified typo as #1: current A = ᎠᏋ (U+13CB 'quv') must be ᎠᏴ (U+13F4 'yv'), the standard 1sg 'I' used in 41 chr cells
...
### #17 `chr` — Same verified typo: current A = ᎠᏋ (U+13CB) must be ᎠᏴ (U+13F4)
...
### #98 `chr` — Same verified typo: current A = ᎠᏋ (U+13CB) must be ᎠᏴ (U+13F4)
...
### #2 `ckb` — Confirmed role reversal vs en (A:My, B:name) and 6-sibling majority assigning A=possessor 'my', B=head noun 'name' (fa B:اسم/A:من, ps A:زما/B:نوم, kn/ml/ta/te all A=my/B=name)
...
### #12 `da` — Confirmed: da #12 C is 'Kirsebærblomstterne' with a doubled t
...
### #23 `et` — Confirmed case-government grammar fix
...
### #28 `fa` — Confirmed person/number agreement error
...
### #49 `fr_be` — Verified sentence-initial-capitalization defect with an exact applied precedent
...
### #60 `fr_be` — Verified missing core verb (_omission_guidelines A
...
### #13 `hak_cn` — Confirmed orthography defect
...
### #34 `hwc` — Confirmed: current hwc B:'mo slow' fuses comparative 'mo' (more) with manner adverb 'slow'
...
### #11 `hy` — Confirmed orthography/typo fix
...
### #89 `ii` — Confirmed single-character typo
...
### #46 `ja_hir` — Confirmed defect: ja_hir currently has E:話した H:んじゃ, fusing the past morpheme た into the verb-stem segment E
...
### #34 `jam` — Confirmed: jam B:'moa sluo' fuses comparative 'moa' with manner adverb 'sluo'
...
### #10 `jv` — Verified defect, identical to id
...
### #68 `kk` — Verified: current kk C:'отыра аламын ба' fuses verb (отыра) + modal can (аламын) + Q-particle (ба) into one segment
...
### #81 `ko` — Confirmed role-label defect
...
### #2 `ku` — Same A/B role reversal as ckb #2: ku had Navê(name)=A, min(my)=B; en + 6 siblings put possessor=A, name=B
...
### #39 `ku` — Confirmed person-agreement error in Kurmanji subjunctive
...
### #64 `ku` — Confirmed: A fused modal Divê with subject ez ('Divê ez')
...
### #30 `ky` — Verified: current ky C:чертет = bare 3sg 'plays', missing the modal 'can' (en E:can; every sibling carries the potential aux: az E:bilir, kk E:алады, tk E:bilýär, he E:יכолה, mt E:tista', ti E:ትኽእл)
...
### #25 `lt` — Confirmed character-level split: inessive 'įmonėje' (at-the-company) chopped into stem D:įmonė + ending B:je
...
### #26 `lt` — Confirmed character-level split: inessive 'parke' (in-the-park) split into stem E:park + ending B:e
...
### #25 `lv` — Confirmed character-level split: Latvian locative 'uzņēmumā' split into stem D:uzņēmum + ending B:ā
...
### #26 `lv` — Confirmed character-level split: locative 'parkā' split into stem E:park + ending B:ā
...
### #64 `lv` — Confirmed character-level split of one morphological form: the Latvian debitive 'jāuzlādē' (must-charge) split into prefix D:jā + verb C:uzlādē
...
### #33 `mr` — Confirmed re-segmentation defect: mr fuses A:hotel (हॉटेलची, genitive) + E:room (खोली, head topic noun) into a single A
...
### #7 `nn` — Confirmed: nn #7 B:'vert vêret' fuses copular/future verb 'vert' (will be → C) with noun 'vêret' (weather → B)
...
### #5 `nn` — Confirmed via codepoint dump: nn #5 B = 'kaffi'+U+00AD(SOFT HYPHEN)+'en'
...
### #45 `no` — Confirmed: no #45 A:'I dette landet' collapses E (prep 'I'), F (demonstrative 'dette'), and A (noun 'landet')
...
### #87 `oc` — Genuine gender-concord error, verified
...
### #99 `pl` — Confirmed gender-agreement defect
...
### #40 `pt_eu` — Verified role-label defect, pure relabel, surface text unchanged
...
### #7 `rm` — Verified fused copula, pure re-segmentation, surface text unchanged ('Damaun è il temp sulegliaus')
...
### #34 `ro` — Verified manner-adverb fusion, exactly the #34 ja_hak precedent in _omission_guidelines A
...
### #67 `scn` — Confirmed: scn #67 has 'Mi allénu' capitalized at position 2 (after A:Iu), not sentence-initial
...
### #2 `so` — Confirmed: 'Magacaygu' = magac(name) + -ay(my) + -gu/-u(subject article), carrying both A(my) and B(name) but labeled only A, leaving the topic noun B (A
...
### #5 `so` — Confirmed mechanical non-adjacent duplicate: D:waa (copula particle) and D:macaan (adjective) split by C:aad u
...
### #18 `so` — Confirmed same defect as so #5: D:waa and D:qaali split by C:aad u (non-adjacent duplicate D), identical 'This X is too ADJ' type
...
### #91 `ti` — Verified: current ti labels ልምዲ (tradition) as D and copula እዩ (is) as C — swapped vs en (C:tradition, D:is) and vs ALL siblings (arc C:מָסוֹרְתָּא D:הִיא, he C:מסורת D:היא, mt C:tradizzjoni D:hija, az C:ənənəsi D:‌dir, kk C:дәстүр D:болып табылады, tk C:däbi D:‌dir)
...
### #81 `tr` — Verified orthography defect: display order is A:Benim first, so C:'Bugün' is mid-sentence
...
### #95 `tr` — Verified orthography defect: display order is D:Ben first, so A:'Bisikletle' is mid-sentence
...
### #95 `uz` — Verified orthography defect: display order D:Men first, so A:'Velosipedda' is mid-sentence
...
### #41 `vec` — Confirmed: 'Adesso' capitalized at position 2 after A:Mi, not sentence-initial
...
### #55 `vec` — Confirmed: 'Tuti i giorni' capitalized mid-sentence after A:Mi
...
### #81 `vec` — Confirmed: 'Ancuo' (today) capitalized at position 2 after A:Mi, not sentence-initial, and not a proper noun
...
### #68 `vi_s` — Verified orthographic defect: D:'được ' carries a stray trailing ASCII space
...
### #96 `yo` — Verified: current yo has E:'tó kàn' AND F:'tó kàn' — two adjacent identical segments (feedback_no_adjacent_same_segments), and the F=intersection noun is missing
...
### #5 `yo` — Verified: current yo A:'Kọfí yìí' fuses B:coffee (Kọfí) with A:this (yìí)
...
### #8 `yo` — Verified: current yo B:'sè oúnjẹ alẹ́' fuses main verb sè (cook, role C) with object oúnjẹ alẹ́ (dinner, B)
...
### #34 `yo` — Canonical manner-adverb case (_omission_guidelines
...
### #47 `yo` — Genuine main-verb/object fusion
...
### #55 `yo` — Genuine verb-object fusion left unaddressed
...
### #57 `yo` — Genuine main-verb/object fusion
...
### #85 `yo` — Genuine modal+verb+object over-fusion
...
### #28 `zh_han` — Confirmed non-adjacent duplicate A
...
### #40 `zh_han` — Confirmed non-adjacent duplicate E
...
### #28 `zh_sc` — Confirmed mislabeled modal
...
### #40 `zh_song` — Confirmed non-adjacent duplicate E
...
### #24 `zh_song` — Confirmed non-adjacent duplicate D
...
### #64 `zh_song` — Verified defect: current zh_song has non-adjacent duplicate C (C:把 
...
## B. 提案・owner確認待ち（蓋然 24 + 要検討 1 = 25件、未適用）

開発者は apply 妥当と判定したが、register/方言/owner判断の余地があるため**未適用**。確認後に適用可能。

### #69 `ko` [蓋然]
- **提案:** `A:걔는  C:부모님  H:‌에게  G:긴  B:편지를  D|E:썼다`
...
### #35 `ko_mid` [蓋然]
- **提案:** `A:내  E:오라비ᄂᆞᆫ  C:디난  D:ᄃᆞᆯ에  B:혼인ᄒᆞ니라`
...
### #34 `th_n` [蓋然]
- **提案:** `A:กะลุนา  C:อู้  D:จ๊ะๆ  B:หน่อย`
...
### #61 `jv` [蓋然]
- **提案:** `A:Dia  B:selalu  C:terlambat  E:teka  D:menyang rapat`
...
### #32 `haw` [蓋然]
- **提案:** `E:Makemake  A:au  C:e ʻai  B:i ka sushi  D:no ka ʻaina awakea`
...
### #32 `mi` [蓋然]
- **提案:** `E:E hiahia ana  A:ahau  C:ki te kai  B:tūhī  D:mō te kai o te awatea`
...
### #85 `mi` [蓋然]
- **提案:** `D:Me  C:inu  A:koe  B:he wai  E:anō`
...
### #34 `fr_af` [蓋然]
- **提案:** `A:S'il te plaît  C:parle  B:plus  D:doucement`
...
### #99 `rm` [蓋然]
- **提案:** `C:Quant  D:costa  A:quest  B:souvenir`
...
### #85 `tpi` [蓋然]
- **提案:** `A:Yu  D:mas  C:dring  E:planti  B:wara`
...
### #34 `en_sg` [蓋然]
- **提案:** `A:Can  C:talk  D:slower  B:a bit can`
...
### #40 `pl` [蓋然]
- **提案:** `E:Czy wie  A:pan/pani  C:gdzie  D:jest  B:poczta`
...
### #89 `bg` [蓋然]
- **提案:** `C:Не  B:мога  D:да намеря  G:паролата  F:за Wi-Fi`
...
### #96 `cu` [要検討]
- **提案:** `A:Молѫ  B:обратите сѧ на десно  C:на  E:приходѧщемъ  F:распѫтии`
...
### #76 `bn` [蓋然]
- **提案:** `A:গান  B:শুনলে  E:আমার  D:খুশি লাগে`
...
### #99 `so` [蓋然]
- **提案:** `C:Immisa  D:ayay  A|B:xusuustani tahay`
...
### #67 `tg` [蓋然]
- **提案:** `A:Ман  B:дар толори варзиш  E:ҳафтае  C:се маротиба  D:варзиш мекунам`
...
### #64 `tk` [蓋然]
- **提案:** `A:Men  B:telefonymy  C:zarýad ber  D:‌meli`
...
### #2 `arc` [蓋然]
- **提案:** `C:הוּא  A|B:שְׁמִי  D:טַנַקַא`
...
### #13 `ig` [蓋然]
- **提案:** `A:Biko  D:nye  B:m  F:iko  C:mmiri`
...
### #13 `sq` [蓋然]
- **提案:** `A:Ju lutem  B:më  D:jepni  F:një gotë  C:ujë`
...
### #93 `chr` [蓋然]
- **提案:** `B:ᎯᎠ  C:ᎦᏙᎯ  D:ᎪᎯᏍᏗ  E:ᎤᏂᎪᎯ  F:ᏗᎾᏙᎵᏍᏗ  G:ᏓᎾᎵᏍᎬᏗ`
...
開発者検証で「現データは正しい / 提案が誤り / 正当な省略 / 保留ポリシー該当」と判定され不適用。

- **#71 `ja_kyo`** [要検討] — Reject. The proposed C:上達 D:させたいどすなぁ is a VOICE-STRATEGY rewrite (intransitive なる -> transitive causative させる) plus removal of もっと, and it CHANGES the surface translation of a natural Kyoto-ben rendering. This is precisely the kind of natural-form/voice canonicalization that _policy_deferred.md #83-
- **#84 `ja_aom`** [要検討] — Reject. あいづァ is a GENUINE Tsugaru は->ぁ contraction (あいつぁ), which the reviewer explicitly acknowledges as authentic Tsugaru-ben. Normalizing to あいづは would erase a legitimate dialect-rendering and CHANGE surface text. The 'irregular full+small kana づ+ァ' objection is weak orthographic nitpicking, not a
- **#30 `ko_bus`** [要検討] — Reject. The proposed C:칠 E:수 있다 CHANGES surface text (친다 -> 칠 수 있다), so this is a translation rewrite, not a re-segment, and falls outside feedback_no_text_change_on_segment_fix. 친다 is a natural Gyeongsang plain-present rendering; the reviewer downgraded to 蓋然 and explicitly conceded that casual Gye
- **#21 `zh_han`** [要検討] — Defect (non-adjacent duplicate E: 將 future + 食 verb with C:與友 between) is real, but the reviewer explicitly grades it 要検討 and states 'no safe mechanical fix' (would require reordering or a spare role-letter in Classical word order). No determinate fix proposed; owner judgment. Defer.
...
- **#34 `lo`** [確実] — The A.4 manner-adverb fusion is linguistically real, BUT the proposed split D:ຊ້າໆ B:ຫນ່ອຍ is NOT surface-preserving as claimed. lo is in NO_SPACE_LANGS; buildFullText joins split segments with no space, so the current literal space (render: ...ພູດຊ້າໆ ຫນ່ອຍ) would be DELETED, producing ...ຊ້າໆຫນ່ອຍ
- **#34 `th_isan`** [確実] — Same as lo #34: th_isan is no-space; proposed split D:ซ้าๆ B:หน่อย (no leading space) would delete the current literal space (render goes from ...เว้าซ้าๆ หน่อย to ...ซ้าๆหน่อย), a surface change despite the 'surface preserved' claim. A.4 motivation is sound but the proposed segments are not text-pr
- **#13 `th`** [蓋然] — th is in NO_SPACE_LANGS. Current F:'น้ำ หนึ่งแก้ว' renders with a literal space (...ให้น้ำ หนึ่งแก้วฉัน). The proposed split C:น้ำ F:หนึ่งแก้ว (no leading space) would render ...น้ำหนึ่งแก้ว..., DELETING that space — a surface change, contradicting the 'surface preserved' claim. This is precisely re
...
- **#39 `vi_nom`** [蓋然] — The structural split D:𥄬 B:𤏬 would be surface-preserving (no internal space; 碎沛𥄬𤏬𠓨𣈜埋 unchanged). However the reviewer explicitly flags that the Nôm character 𥄬 (U+2512C) is used elsewhere in this very dataset (#73 vi_nom D:𥄬 = vi ngủ 'sleep') for the OPPOSITE meaning of the needed dậy 'wake 
- **#43 `th_isan`** [要検討] — Reviewer grades 要検討 and presents two options (keep documented prep-drop as E:ฮ้าน vs supply A:ที่), explicitly leaving it to owner judgment. The A|E composite is surface-neutral (text ฮ้าน unchanged either way), but no single determinate fix is asserted, and th_n already drops A entirely (precedent 
- **#84 `fj`** [確実] — The hard defect is real and confirmed: B:na syõro contains 'õ', and #84 is the ONLY fj cell in the entire dataset with a tilde/circumflex (Standard Fijian uses plain a-e-i-o-u). However the PROPOSED correction is not verifiable. The reviewer's own confidence is 要検討 and they explicitly write 'the exa
- **#22 `sm`** [確実] — Reject: the proposed segments are defective on two counts the reviewer themselves flagged (蓋730/蓋然, 'owner may prefer leaving the NP intact'). (1) The proposed array [...C:gagana, F:se, F:fou] concatenates to 'gagana se fou', changing the surface text from the current 'se gagana fou' — this violates
- **#41 `ca`** [蓋然] — Cross-sibling consistency flag, not a hard defect. The reviewer self-rates it 蓋然 and explicitly says Catalan adj-after-noun NP fusion is 'a defensible owner choice, not a hard error'. Critically, review #49 examined #41 and ruled the whole indefinite-article / NP-internal split question (cells #22/#
- **#42 `lad`** [蓋然] — The non-adjacent duplicate E (E:un ... E:vestido with D:kolorado between) is technically present, but the proposed remedy reorders the surface text ('un kolorado vestido' → 'un vestido kolorado'), which the reviewer admits is 'register/orthography-debatable' (蓋30). This contradicts the established #
- **#25 `oc`** [蓋然] — Although surfacing C:una is text-preserving and aligns with en (C:a) and most siblings, this is exactly the indefinite-article split-vs-fuse question that review #49 explicitly flagged as an unsettled grand-convention matter ('数百セル規模...closed-review で裁定推奨'), covering Romance/Germanic NP article segm
- **#12 `oc`** [要検討] — Reviewer-marked 要検討 and explicitly 'dialect/register dependent, hence owner judgment'. Wiktionary confirms 'polit/polida' = pretty (fem pl 'polidas') and that '-òt' is a legitimate productive Occitan diminutive/affective suffix. 'polidòtas' = 'rather/cutely pretty' is a real, grammatical Occitan wor
- **#18 `pt_br`** [要検討] — Owner-judgment / policy item, and against a prior accepted state. The reviewer self-rates it 要検討 and explicitly 'flagged for owner judgment on whether role tracks function vs. linear position' — a cross-sibling role-consistency policy question. Moreover review #55 examined pt_br #18 and explicitly m
- **#21 `de`** [蓋然] — DIRECT CONFLICT with a deliberate prior hand-made fix. Review #55 examined de #21 (then '[E:esse][B:heute Abend][C:mit Freunden][D:zu Abend]'), explicitly considered BOTH inserting [D:Abendessen] AND deleting the dinner element, and chose to DELETE 'zu Abend', settling on the native-standard form 'I
- **#1 `got`** [確実] — Facts verified (90 got cells in Gothic script U+10330–U+1034F, 10 in Latin transliteration: exactly ids 1,9,13,17,28,89,93,94,96,98). But this finding is explicitly NOT actionable as a single-cell edit: the reviewer states segments are left UNCHANGED on purpose, marks it 要検討, declines to fabricate G
- **#66 `non`** [確実] — The two non-adjacent C segments (C:er ... C:hulið around A:fjallit) are genuine Old Norse V2 syntax, but the proposed fix is malformed: its segments array is [B:'Af snjó'][C:'er fjallit hulið'][A:'fjallit'], which puts 'fjallit' inside the merged C AND keeps A:'fjallit', producing surface 'Af snjó e
- **#42 `nn`** [確実] — The proposed fix [A:Ho][B:alltid][C:'har på seg'][D:'ei raud'][E:kjole] reorders the surface to 'Ho alltid har på seg...' which differs from the current 'Ho har alltid på seg...' and is ungrammatical Nynorsk (V2 requires the finite verb 'har' in second position, before the adverb 'alltid'). This is 
- **#90 `en_au`** [蓋然] — The current surface 'learned himself programming' is a genuine nonstandard Australian/colloquial reflexive ditransitive ('learn someone something' = teach), consistent with the heavy dialectal coloring of the other variants (en_app 'all by hisself', en_yk 'by 'is sen', en_south 'done learned', hwc '
- **#5 `lt`** [確実] — The 'non-adjacent duplicate D' (D:yra ... C:labai ... D:skani) is NOT a defect: review #53 issue 14 explicitly ruled this pattern legitimate — feedback_no_adjacent_same_segments bans only ADJACENT same-letter, and here C separates the two D's, so it is non-adjacent and legal ('C を挟んで非隣接なので合法'). lt #
- **#5 `lv`** [確実] — Same as lt #5: the non-adjacent D-twice pattern (D:ir/C:ļoti/D:garšīga) was declared legitimate by review #53 issue 14 (non-adjacent, so not a feedback_no_adjacent_same_segments violation). lv #5 correctly mirrors the en/de/fr/it master. The #87 C-fusion the finding cites as canonical is itself the 
- **#18 `lt`** [確実] — Same construction as #5 lt. Non-adjacent D-twice (D:yra/C:per/D:brangi) is legitimate per review #53 issue 14; lt #18 mirrors the en master (D:is/C:too/D:expensive). Not a no-adjacent violation (C separates the D's). The #87 C-fusion cited as the model is the documented outlier. Rejecting.
...
- **#89 `fa`** [要検討] — Reject. Reviewer self-flagged 要検討 and noted the را re-attachment 'is debatable and may be deliberate authorial chunking'. The proposal couples two surface-changing edits: moving را onto the object (F:Wi-Fi را) and unifying نمی توانم→نمی‌توانم (space→ZWNJ). While the unified negated-modal has sibling
- **#24 `am`** [要検討] — Reject as policy-deferred. The finding explicitly conditions itself on policy item #84-C ('apply only if that policy is adopted'). #84-C (bound-pronoun B|C composite to surface bound subject/object morphology) is still listed in _policy_deferred.md with Status 'policy decision needed' and has NOT be
- **#99 `ar_tn`** [確実] — The proposed fix rewrites the surface ('بقدّاش هالسوفنير هاذا' → 'بقدّاش هاذا السوفنير'), changing both word order and text — not a pure relabel. The premise that 'a demonstrative cannot be doubled' is wrong for Maghrebi: proclitic هاـ + noun + postposed هاذا is a legitimate Tunisian circumfixal/rei
- **#43 `az`** [蓋然] — Reviewer self-rated 要検討 and explicitly states this is 'an owner-policy consistency call rather than a hard error': az written standard keeps genitive case morphology fused (restoranın). The kk/ky/tk ZWNJ A-segment split is one family convention, but az legitimately differs — this is style/notation n
- **#10 `mnc`** [確実] — The finding correctly identifies that mnc #10 is a wrong translation (currently 'I have read this book', missing 'you/English/speak/can'), but it is self-flagged 要検討 and explicitly says: 'Flagged for Manchu native-speaker confirmation of the exact form for English before applying, per the iu native-
- **#42 `yo`** [確実] — Already deliberately KEEP'd, twice, in review #29 (round-12 worker + round-13 reviewer Dr. Okeke-Wambui, both 'KEEP 妥当'). Yoruba expresses habitual 'always' as a double construction: preverbal habitual marker 'máa ń' + emphatic adverb 'nígbà gbogbo', both tagged B. The exact proposed fix (fuse máa ń
- **#61 `yo`** [確実] — Same as #42: explicitly KEEP'd in review #29 (round-12 + round-13, both 'KEEP 妥当', '#42 と同一構造'). The double 'always' (preverbal habitual máa ń + emphatic adverb nígbà gbogbo) is the documented Yoruba construction, both halves B. Fusing máa ń into E:dé was the considered-and-rejected option (creates 
- **#55 `sw`** [確実] — The F:vikombe…F:vya structure is a deliberate design, not a defect. Review #29 created it (round-4) and explicitly re-affirmed it (round-13, 'KEEP 妥当'): Swahili 'vikombe … vya' = the discontinuous analytic measure phrase 'cups … of', both halves tagged F (container/measure) bracketing the numeral E:
- **#22 `yo`** [確実] — Proposed segments are linguistically wrong and self-contradictory. Yoruba order is èdè(language) tuntun(new) kan(a). The finding's own text says to put 'new' (tuntun) in F, but its segments array is C:'èdè tuntun' (language+new still fused) + F:'kan' (mislabels the article 'a' as F). Siblings put C=
- **#35 `sq`** [要検討] — Policy-deferred, not actionable. The reviewer themselves marked this 要検討 and explicitly 'left unchanged pending policy', drawing a direct parallel to the deferred Celtic VSO double-letter exception (_policy_deferred.md #82-C, recommendation B: document as permitted exception, do NOT force-merge). Th
- **#95 `qu`** [要検討] — Reject (do not auto-apply; owner review). The casing observation is linguistically plausible — Bicicletapi is a mid-sentence common-noun loanword (D:Ñuqam leads) and siblings lowercase it (myn A:'ich bicicleta', gn A:'bicicleta-pe'). BUT this cell was hand-curated in closed review #36 (issue #9 / ap
- **#34 `tlh`** [要検討] — Reject. (1) Cross-sibling convention contradicts the fix: across this sentence, languages with only a SINGLE manner word fold it into B, not D (pl B:wolniej, fi B:hitaammin, uk B:повільніше, cy B:yn arafach, hit B:manninkuwandaš, akk B:arḫiš lā, sux B:búr-búr-re, ine B:*ml̥dú-teros). Existing tlh B:

---

*Rally #111 — generated 2026-06-16. data.js: 確実78件適用済み。蓋然24+要検討1は owner 確認待ち。*
