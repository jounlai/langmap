...
Findings: 4 · applied 4 · rejected 0 · skipped 0. Edits applied to live words/*.js with `node validate_wordmap_data.js` passing.

---

## Reviewer round-2 response (再評価 round-2)

All 4 findings were APPLIED in round-1. Verified each against current live data; all match the round-1 proposed values. `node validate_wordmap_data.js` → PASS.

- ✓ ACCEPT — verified live: words/sun.js `yo: ["oòrùn", "oòɾũ̀"]` — proposed nasal vowel [ũ̀] present.
- ✓ ACCEPT — verified live: words/heart.js `yo: ["ọkàn", "ɔkã̀"]` — proposed [ã̀] present, spurious /n/ removed.
- ✓ ACCEPT — verified live: words/one.js `yo: ["ọ̀kan", "ɔ̀kã"]` — proposed [ã] present, spurious /n/ removed.
- ✓ ACCEPT — verified live: words/tree.js `szl: ["strōm", "strɔm"]` — spurious tilde removed; oral [ɔm].
...
|------|---------|
| 1. yo — sun — ipa | ✓ ACCEPT |
| 2. yo — heart — ipa | ✓ ACCEPT |
| 3. yo — one — ipa | ✓ ACCEPT |
| 4. szl — tree — ipa | ✓ ACCEPT |
