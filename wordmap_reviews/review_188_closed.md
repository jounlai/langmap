...
Read-only verification against current live data in words/*.js. `node validate_wordmap_data.js` → PASS.

- ✓ ACCEPT — verified live: words/tree.js:910 `gmy: ["𐀈𐀬", "doru"]` — APPLIED proposed value present (do-ru = δόρυ 'tree/wood'; old do-ka/dorka gone).
- ✓ ACCEPT — verified live: words/heart.js:517 `got: ["𐌷𐌰𐌹𐍂𐍄𐍉", "hɛrtoː"]` — APPLIED; length mark removed, only final ō long (Gothic breaking, Wright §22).
- ✓ ACCEPT — verified live: words/mother.js:523 `got: ["𐌰𐌹𐌸𐌴𐌹", "aiθiː"]` — REJECTED; cell unchanged, original diphthongal reading preserved (monophthong /ɛː/ vs /ai/ genuinely contested, marked discuss).
...
| item | verdict |
|------|---------|
| 1. gmy tree → doru | ✓ ACCEPT (applied) |
| 2. got heart → hɛrtoː | ✓ ACCEPT (applied) |
| 3. got mother → ɛːθiː | ✓ ACCEPT (rejected, unchanged) |
